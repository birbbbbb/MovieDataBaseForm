     ...
     .then(({ status, json }) => {
         ...
         if (status === 400) {
            ...
            for(let i = 0; i < json.data.length; i+=2){
                ...
                table += `<td>${row.runtime}<td>`
                table += `<td>${row.director}<td>`
                table += `<td>${row.actors}<td>`
                table += `<td>${row.summary}<td>`
                table += `<td>${row.trailer}<td>`
                table += `<td><img src="../Server/${json.files.poster[0].path}"/><td>`
                table += `<td><img src="../Server/${json.files.video[0].path}"/><td>`
                table += `</tr>`
